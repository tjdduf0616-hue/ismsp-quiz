...
ISMS-P 인증심사원 시험 대비용 모바일 웹 퀴즈 앱입니다. 고정 문제은행 + AI 실시간 생성 + 모의고사 + 회원별 기록(별표·오답노트)을 제공합니다.
...
- 카테고리를 새로 추가하려면 `index.html`의 `CATS`, `CAT_LABEL`, (AI 생성용) `DOC_CONTEXT`도 함께 수정
...
## 주의
- API 키 등 비밀값을 코드에 하드코딩하지 마세요(공개 저장소). AI 생성용 Claude API 키는 앱 설정에서 사용자 브라우저에만 저장됩니다.
